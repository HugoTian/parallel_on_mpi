  ...
  // perform matrix multiplies of r[0] and r[1]
   ...
   for(x = rank; x < matrix_dimension_size; x = x +size) //divide the task in multiple processes
    ...
    if(rank != 0)
    {
        ...
        MPI_Send(&result[0][x*matrix_dimension_size + 0], matrix_dimension_size, MPI_INT, 0, 10+x, MPI_COMM_WORLD);//send calculated rows to process with rank 0
    }
    ...
    if(rank == 0)
    {
        for(y = 1; y < size; y++)
        {
            ...
            {
                MPI_Recv(&result[0][x*matrix_dimension_size+0], matrix_dimension_size, MPI_INT, y, 10+x, MPI_COMM_WORLD, &status);//receive calculated rows from respective process
            }
        }
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    */
    ...
    while( i < num_arg_matrices) {
      ...
      {
          for(x = rank; x < matrix_dimension_size; x = x+size)
          MPI_Send(&result[n ^ 0x1][x*matrix_dimension_size + 0], matrix_dimension_size, MPI_INT, 0, 10+x, MPI_COMM_WORLD);//send calculated rows to process with rank 0
      }
         ...
         for(y = 1; y < size; y++)
          {
              for(x = y; x < matrix_dimension_size; x = x+size)
              {
                  MPI_Recv(&result[n ^ 0x1][x*matrix_dimension_size+0], matrix_dimension_size, MPI_INT, y, 10+x, MPI_COMM_WORLD, &status);//receive calculated rows from respective process
             }
          }
      }
